...
fn main() {
    println!("╔════════════════════════════════════════════════════════════════╗");
    println!("║     Entrenamiento Automático de Parámetros V8.7               ║");
    println!("║     (Onion Peeling - Multi-Layer Convex Hull)                 ║");
    println!("╚════════════════════════════════════════════════════════════════╝\n");
    ...
    // Grid de búsqueda
    let k_values = vec![4, 6, 8, 10, 12, 16];
    let w_values = vec![0.0, 0.25, 0.5, 0.75, 1.0];

    let total_combinations = k_values.len() * w_values.len() * w_values.len();
    println!("Grid de búsqueda:");
    println!("  k_neighbors: {:?}", k_values);
    println!("  w_angle: {:?}", w_values);
    println!("  w_cost: {:?}", w_values);
    println!("  Total combinaciones: {}\n", total_combinations);

    let start_total = Instant::now();
    let mut best_params = V87Params::default();
    let mut best_avg_error = f64::MAX;
    let mut results = Vec::new();
    let mut count = 0;

    for &k in &k_values {
        for &w_angle in &w_values {
            for &w_cost in &w_values {
                count += 1;
                ...
                if avg_error < best_avg_error {
                    ...
                    println!(
                        "[{}/{}] Nuevo mejor: k={}, w_angle={:.2}, w_cost={:.2} → Error: {:.2}%",
                        count, total_combinations, k, w_angle, w_cost, avg_error
                    );
                }

                if count % 50 == 0 {
                    let elapsed = start_total.elapsed().as_secs();
                    let remaining = (elapsed as f64 / count as f64) * (total_combinations - count) as f64;
                    println!(
                        "  Progreso: {}/{} ({:.1}%) - Tiempo transcurrido: {}s - Estimado restante: {}s",
                        count,
                        total_combinations,
                        (count as f64 / total_combinations as f64) * 100.0,
                        elapsed,
                        remaining as u64
                    );
                }
    ...
    println!("\n════════════════════════════════════════════════════════════════");
    println!("Entrenamiento completado en {:.2}s", total_time.as_secs_f64());
    println!("════════════════════════════════════════════════════════════════\n");
    ...
    // Guardar resultados
    let mut file = File::create("assets/v87_training_results.txt").unwrap();
    writeln!(file, "# Resultados de entrenamiento V8.7").unwrap();
    writeln!(file, "# Tiempo total: {:.2}s", total_time.as_secs_f64()).unwrap();
    writeln!(file, "# Total combinaciones evaluadas: {}", total_combinations).unwrap();
    ...
    writeln!(file, "\n# Top 10 mejores configuraciones:").unwrap();

    results.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap());
    for (i, (params, error)) in results.iter().take(10).enumerate() {
        writeln!(
            file,
            "#{}. k={}, w_angle={:.2}, w_cost={:.2} → Error: {:.2}%",
            i + 1,
            params.k_neighbors,
            params.w_angle,
            params.w_cost,
    ...
    println!("\n✓ Resultados guardados en: assets/v87_training_results.txt");
}
